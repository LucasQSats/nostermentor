...
function consultar(url) {
  return new Promise((resolve) => {
    ...
    ws.onmessage = (m) => { let msg; try { msg = JSON.parse(m.data); } catch (e) { return; }
      if (msg[0] === 'EVENT' && msg[2] && msg[2].kind === 15128 && msg[2].pubkey === pubkey) r.eventos.push({ id: msg[2].id, created_at: msg[2].created_at, paths: msg[2].tags.filter(t => t[0] === 'path').length });
...
(async () => {
  ...
  console.log(JSON.stringify({ pubkey, por_relay, mais_recente: mais(rs.flatMap(r => r.eventos)), respondidos: rs.filter(r => r.estado === 'ok').length, total: rs.length }));
  process.exit(0);
})();
